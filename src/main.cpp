...
int main(int argc, char* argv[])
{
  ...
  FileUtil::SetApplicationDir(std::string(getenv("HOME")) + std::string("/.proton"));
  ...
  if (!dirLock.IsLocked())
  {
    std::cerr <<
      ...
      "       only one proton session per account/confdir is supported.\n";
    return 1;
  }
  ...
  if (!isDirInited)
  {
    ...
    if (storedVersion != dirVersion)
    {
      ...
      {
        std::cout << "Config dir " << FileUtil::GetApplicationDir() << " is incompatible with this version of proton\n";
...
{
  std::cout <<
    "proton is a minimalistic terminal-based chat client with support for\n"
    "telegram.\n"
    "\n"
    "Usage: proton [OPTION]\n"
    "\n"
...
{
  std::cout <<
    "proton v" << AppUtil::GetAppVersion() << "\n"
    "\n"
    "proton is distributed under the MIT license.\n";
}
